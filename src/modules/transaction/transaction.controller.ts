...
export class TransactionController {
    ...
    try {
      ...
      try {
        if (files && Object.keys(files).length > 0) {
          ...
          const savedFiles = await this.filesHelperService.saveTransactionFiles(
            files,
            transaction.id,
          );
          ...
          transaction = await this.transactionService.updateTransaction(
            transaction.id,
            { ...transaction, pictures: savedFiles },
          );
        }
      ...
      this.logger.error(
        `[TransactionController] [create] Error: ${error.message}`,
      ); // Log error
      throw new HttpException(
        error.message === "Image Upload Issue"
  ...
  async update(
    ...
    try {
      ...
      this.logger.error(
        `[TransactionController] [update] Error: ${error.message}`,
      ); // Log error
  ...
  async findAll(): Promise<response<Transaction[]>> {
    try {
      ...
      this.logger.error(
        `[TransactionController] [findAll] Error: ${error.message}`,
      ); // Log error
      throw new HttpException(
  ...
  async findPastTransaction(
    ...
    try {
      ...
      this.logger.error(
        `[TransactionController] [findPastTransaction] Error: ${error.message}`,
      ); // Log error
      throw new HttpException(error.message, HttpStatus.BAD_REQUEST); // Handle error
    }
  }
  ...
  async remove(@Param("id") id: string): Promise<response<void>> {
    ...
    try {
      ...
      this.logger.error(
        `[TransactionController] [remove] Error: ${error.message}`,
      ); // Log error
      throw new HttpException(
        Messages.transaction.removeFailure(transactionId),
        HttpStatus.BAD_REQUEST,
      ); // Handle error
    }
  }
  ...
  async getTransactionsByDate(
    ...
    try {
      ...
      this.logger.error(
        `[TransactionController] [getTransactionsByDate] Error: ${error.message}`,
      );
      throw new HttpException(
  ...
  async uploadExcel(
    ...
    try {
      const errorArray = await this.transactionService.processTransaction(
        file,
        "transaction",
      );
  ...
  async uploadFineExcel(
    ...
    try {
      ...
      if ("fine" in fileResponse) {
        // Save transaction to the database
        const filteredTransaction = fileResponse.fine.filter(
          (item) => item !== undefined,
        ); // Assuming you have a createBulk method
      } else {
