...
export class VehicleService {
  ...
  async findOne(id: string): Promise<Vehicle> {
    try {
      ...
      throw new InternalServerErrorException(
        `Failed to find vehicle with id: ${id}`,
      );
    }
  }
  ...
  async findByVehicleNo(vehicleNo: string): Promise<Vehicle> {
    try {
      ...
      this.logger.error(`[VehicleService] [findOne] Error: ${error.message}`);
      throw new InternalServerErrorException(
        `Failed to find vehicle with id: ${vehicleNo}`,
      );
    }
  }
  ...
  async updateVehicles(vehicles: Vehicle[]): Promise<void> {
    try {
      ...
      this.logger.error(
        `[VehicleService] [updateVehicles] Error: ${error.message}`,
      );
  ...
  async checkRelation(checkRelationDto: VehicleDto): Promise<{
    ...
    if (missingFields.length > 0) {
      throw new BadRequestException(
        `Invalid or missing fields: ${missingFields.join(", ")}`,
      );
    }
    return {
      vehicleType,
  ...
  async getFilteredVehicles(
    ...
    if (vehicleType) {
      if (!vehicleType || vehicleType.split("-").length !== 2) {
        throw new Error(
          `Invalid vehicleType format. Expected 'name - fuel', got '${vehicleType}'`,
        );
      }
  ...
  async getVehicleCountByModel() {
    return this.vehicleRepository
      ...
      .addSelect("COUNT(vehicle.id)", "vehicleCount")
      .addSelect(
        "SUM(CASE WHEN vehicle.status = :available THEN 1 ELSE 0 END)",
        "available",
      )
      .addSelect(
  ...
  async getVehicleCountByType() {
    return this.vehicleRepository
      ...
      .select(
        "CONCAT(vehicleType.name, ' - ', vehicleType.fuel)",
        "vehicleTypeName",
      )
      .addSelect("COUNT(vehicle.id)", "vehicleCount")
      .addSelect(
